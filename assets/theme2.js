window.slate = window.slate || {}, window.theme = window.theme || {},
	function(t, e) {
		...
		}), S.ready.then = N.then, "complete" === g.readyState || "loading" !== g.readyState && !g.documentElement.doScroll ? t.setTimeout(S.ready) : (g.addEventListener("DOMContentLoaded", H), t.addEventListener("load", H));
	...
	*/
    ...
    jQuery.fn.prepareTransition = function() {
        return this.each((function() {
            ...
            var i = 0;
            jQuery.each(["transition-duration", "-moz-transition-duration", "-webkit-transition-duration", "-o-transition-duration"], (function(t, n) {
...
PaloAlto.ThemeProduct = {
	...
	return ct.prototype = $.extend({}, ct.prototype, {
        ...
        getUpsellCz() {
          /*var material = document.getElementById("upsellcz").innerText;*/
          var upsellType = document.getElementById("upsellType").innerText;
          
          if(upsellType != ""){
           // fetch(theme.routes.root + "collections/cross-selling/?filter.v.availability=1"+material+"&upsellType="+upsellType+"&section_id=section-upsellcz")
             fetch(theme.routes.root + "collections/cross-selling?filter.v.availability=1&section_id=section-upsell2")
               .then(response => {
                  if (!response.ok) {
                      throw new Error('Network response was not ok');
                  }
                  return response.text(); // Assuming response is text, use json() if it's JSON
              })
              .then(data => {
                 document.getElementById('upsellDiv').innerHTML = data;
                 $('.main-carousel3').children('.cell').sort(function(a, b) {return $(a).data('order') - $(b).data('order');}).appendTo('.main-carousel3');
                ...
                var parentDiv = $(".main-carousel3");
                  var cellDivs = parentDiv.find(".cell");
                  var maxHeight = 0;
                  cellDivs.each(function() {
                   ...
                   if (cellDivs.length == 0) { $('#upsellDiv').addClass('notMoreProduct');}else{ $('#upsellDiv').removeClass('notMoreProduct');}
                   if (cellDivs.length > 1) {
                      if ($(document).width() > 768){
                       $(".main-carousel3 .carousel-cell").css({"margin-right": "56px"});
                       $(".main-carousel3 .cell").css({"width": "85%","height":maxHeight});
                      }else{
                        $(".main-carousel3 .carousel-cell").css({"margin-right": "0px"});
                        $(".main-carousel3 .cell").css({"width": "100%","height":maxHeight});
                      }  
                    } else {
                    
                     if ($(document).width() > 768){
                      $(".main-carousel3 .carousel-cell").css({"margin-right": "20px"});
                      $(".main-carousel3 .cell").css({"width": "100%","height":maxHeight});
                     } else {
                         $(".main-carousel3 .carousel-cell").css({"margin-right": "0px"});
                       $(".main-carousel3 .cell").css({"width": "100%","height":maxHeight});
		...
		renderCartDrawer(t = !0) {
			...
			e && (this.cartDrawer.innerHTML = e.innerHTML, this.assignArguments(), this.initQuantity(), this.cartEvents(), this.cartDrawer.querySelector(j) && (this.pairWithArray = JSON.parse(this.cartDrawer.querySelector(j).innerText.split("=")[1].replace(";", ""))), this.getPairProducts(), this.renderPairProducts(), this.pairProductSkipEvent(), PaloAlto.initCollapsible(), this.cartDrawerToggle = this.cartDrawer.querySelector(l), this.cartDrawerToggle.addEventListener("click", this.cartDrawerToggleClickEvent), this.isCartDrawerLoaded = !0, document.dispatchEvent(new CustomEvent("theme:cart:loaded", {
				bubbles: !0
			})),this.getUpsellCz(), t && this.openCartDrawer())
          $.getScript('https://wrapin.prezenapps.com/public/js/load-wrapin-v5-app.js');
		},
		openCartDrawer() {
    ...
    jQuery.fn.prepareTransition = function() {
        ...
        jQuery(t).prepareTransition().css("height", e).attr("aria-hidden", i).toggleClass(n, !i), r && clearTimeout(r), 0 == e && (r = setTimeout((function() {
			jQuery(t).css("height", "")
		}), 500)), i ? o = !1 : setTimeout((function() {
			jQuery(t).css("height", "auto"), o = !1
		}), 500)
	}
	jQuery("[data-collapsible-trigger]").off("click keyup").on("click keyup", s), $(t).off("click keyup").on("click keyup", s)
	...
	return u.prototype = $.extend({}, u.prototype, {
		...
		staggerChildAnimations() {
			this.disclosure.querySelectorAll(n).forEach(((t, e) => {
              ...
              t.style.transitionDelay = 60 * e + 10 + "ms"
			}));
...
PaloAlto.CustomScrollbar = function() {
	...
	initListeners: function() {
		...
		$(this.selectors.searchForm).on("keyup", this.selectors.searchInput, PaloAlto.debounce((function(e) {
            ...
            var search_head = $(".site-header").height() + "px";
             $('.nav-search.nav-search--is-visible').height(search_head);
			e.preventDefault(), t.open(),$('#search-field').focus();
	...
	injectHTML(t, e) {
		t.innerHTML += e
      var search_ht = $('.nav-search__scroller').height() ;
      var newht = $('.nav-search__scroller').height() + 120;
      //console.log('newheight',newht);
       $('.nav-search.nav-search--is-visible').height(newht + "px");
	},
	renderNoResult() {
		const t = {
	...
	return h.prototype = $.extend({}, h.prototype, {
		...
		headerState: function(e) {
			const i = parseInt(this.header.dataset.height || this.header.offsetHeight),
              ...
              var abbar = document.getElementById('Announcement--announcement-bar').offsetHeight+ 'px'; 
             if (document.querySelector("body.template-product") && !document.querySelector('.select-popout').classList.contains('hasSize')) {
              
				"fixed" === this.position && (r > o ? (document.querySelector('div .cz_fixingButton section').style.bottom = '0') : (document.querySelector('div .cz_fixingButton section').style.bottom = '-65px'))
  			  }
            "fixed" === this.position && (r > o ? (document.querySelector('.nav-search').style.top = '0px') : (document.querySelector('.nav-search').style.top = abbar))
...
}), PaloAlto.updateHash = function(t) {
...
showAnimations && AOS.init({
	...
	"" != $(".cz_single_product_size_selector").val() && ($(".select-popout").css({
		"border-width": "0px"
	}), $(".select-popout").addClass("hasSize"), $('div .cz_fixingButton section').css('bottom', '0'), $(".product__submit__add").css({
		"background-color": "#fff",
		color: "#000"
	}))
})), $(".product__submit__add").hover((function() {
	//"Select size" == $(".select-popout__value").text() ? ($(this).addClass("pink"), setTimeout(function() {$(".product__submit__add").children().text("PLEASE SELECT SIZE")}, 150)) : ($(".product__submit__add").children().text("ADD TO BAG"), $(this).removeClass("pink"))
   		"Select size" == $(".select-popout__value").text() ? ($(this).addClass("pink"), $(".product__submit__add").children().text("PLEASE SELECT SIZE")) : ($(".product__submit__add").children().text("ADD TO BAG"), $(this).removeClass("pink"))
...
})),$("#material_pop").click((function(t) {
	t.preventDefault(), $("#myModalCz2").css({
		display: "block"
	})
...
$(".imgLight img").click((function(e) {  
	...
	$('.imgLight img').toggleClass("extZoom"), $(".zoomLightBox").toggleClass("inZoom"), $(".imgLight img").css("max-width", "unset")
	var outerContent = $('.imgLight');
	var innerContent = $('.Czlt img');
	outerContent.scrollLeft( (innerContent.width() - outerContent.width()) / 2);
	outerContent.scrollTop( (innerContent.height() - outerContent.height()) / 1.25);
...
$(".closeTopBar").click((function() {
	...
	outerContent.scrollTop( (innerContent.height() - outerContent.height()) / 1.25);
      ...
      var outerContent = $('.imgLight');
      var innerContent = $('.Czlt .zoomImg');
      
      var indexToScrollTo = c_thumb-1; // This is the index you want to scroll to (0-based index)
      
      if (indexToScrollTo >= 0 && indexToScrollTo < innerContent.length) {
        var targetImg = innerContent.eq(indexToScrollTo);
        var targetOffset = targetImg.offset().top - outerContent.offset().top + outerContent.scrollTop();
...
/*$('.nav-search__input').on('keyup',function(){
   var search_ht = $("#search-results").height() +150+ "px";
    $('.nav-search.nav-search--is-visible').height(search_ht);
});*/
$('.nav-search__close').on('click',function(){
    var search_head = $(".site-header").height() + "px";
    $('.nav-search.nav-search--is-visible').height(search_head);
});
$('.mobile-menu a.mobile-menu__button.searchCZ').on('click',function(e){
  e.preventDefault();
});
  /*if (document.cookie.indexOf('cookie_emarket') == -1){
     $('.close-sideways').parent().toggleClass('close_emarket');
  }*/
function setcookEmarket() {
	var t = new Date;
	t.setHours(23, 59, 59, 0), document.cookie = "cookie_emarket=" + escape("1") + ";path=/;" + (null == t ? "" : "; expires=" + t.toGMTString())
}

    function changeOpacityBasedOnScroll() {
            var siteFooterHeight = $(".site-footer").height()-500;
            var windowHeight = $(window).height();
            var scrollPosition = $(window).scrollTop();
            
            // Calculate the distance from the scroll position to the footer
            var distanceToFooter = document.documentElement.scrollHeight - scrollPosition - windowHeight;
            
            // Calculate the opacity based on the distance to the footer
      
          

            var opacity = 1 - (distanceToFooter / siteFooterHeight);
            var opacity_head = 1 - opacity; // Opposite of opacity
            opacity = Math.max(0, Math.min(1, opacity)); // Ensure opacity is within [0, 1]
            
            // Set the opacity of the site footer
            $(".footerLogo").css("opacity", opacity);
      
            $(".logo").css("opacity", opacity_head);
        } 


 $(document).on("click", "#stamped-badge-2", (function() {   
       window.open('https://www.anoncph.com/a/review/write');
 }));
...
$('.cart__toggle').on('click', function() {
  ...
  $(".product_block.plateOuter.forMobile ul").each(function(index, ulElement) {
    var numberOfLiElements = $(ulElement).find("li").length;
    if (numberOfLiElements > 6) {
      $(ulElement).addClass("narrow-ul");
      $(ulElement).after("<span></span>");
    }
  });
  
      $(".bcorp-icon").click(function() {
             $("#myModalCz_iframe_bcorp").css({"display": "block"});
          });
          $(".emaerket-cert").click(function() {
            // $("#myModalCz_iframe_emaerket").css({"display": "block"});
             $(".emaerket-widget-hide-print").trigger("click");
          });
  $('.close-sideways').on('click', function() {
    setcookEmarket();
    $(this).parent().toggleClass('close_emarket');
    const parentElement = $(this).parent();

    if (parentElement.hasClass('close_emarket')) {
      
      $('.close-sideways').removeClass('animate_down').addClass('animate_up');
     
    } else {
       $('.close-sideways').addClass('animate_down').removeClass('animate_up');
    }
  });
 ...
 /* $('a[href*="#"]').on('click', function (event) {
  ...
  $('.collection__products .product_block.plateOuter.onCollectionPage ul.colorPlate li a').on('click', function (event) {
    event.preventDefault();
    ...
    var discount = $(this).data('discount');
      //var htm = price+"<s>"+compare_price+"</s>;
   var htm = price + " <s>" + compare_price + "</s>";
      // Reference to the product__media element
      $('div.image').removeClass('currentProduct');
  var productMedia = $(this).closest('.product-grid-item').find('.product-grid-item__image .product__media');
  $(this).closest('div.image').addClass('currentProduct');
  $(this).closest('.product-grid-item').find('a.product__media__holder').attr('href', link);    
  $(this).closest('.product-grid-item').find('.product-grid-item__info a.product-grid-item__title').attr('href', link);    
  $(this).closest('.product-grid-item').find('.product-grid-item__info a.product-grid-item__price').attr('href', link);          
  $(this).closest('.product-grid-item').find('.product-grid-item__info a.product-grid-item__price').html(htm);    
  productMedia.fadeOut('fast', function () {
    productMedia.css('background-image', 'url(' + imageUrl + ')').fadeIn('fast');
  });
  ...
  $('.homeFeaturedCollection .featured-collection__container .product_block.plateOuter ul.colorPlate li a').on('click', function (event) {
    event.preventDefault();
    var imageUrl = $(this).data('image');
    console.log(imageUrl);
    var link = $(this).data('url');
    $('div.image').removeClass('currentProduct');
    var productMediaFighlights = $(this).closest('.product-grid-item').find('.product-grid-item__image .product__media__image .product__media ');
    $(this).closest('div.image').addClass('currentProduct');
    $(this).closest('.product-grid-item').find('a.product__media__holder').attr('href', link);    
    $(this).closest('.product-grid-item').find('.product-grid-item__info a.product-grid-item__title').attr('href', link);    
    $(this).closest('.product-grid-item').find('.product-grid-item__info a.product-grid-item__price').attr('href', link); 
    productMediaFighlights.fadeOut('fast', function () {
    productMediaFighlights.css('background-image', 'url(' + imageUrl + ')').fadeIn('fast');
  });
    
  });

      $('.collection__products .product_block.plateOuter.forMobile ul.colorPlate li a').on('click', function (event) {
    event.preventDefault();
    var imageUrl = $(this).data('image');
    var link = $(this).data('url');
    var price = $(this).data('price');
  ...
  $('.product_block.plateOuter.forMobile span').on('click', function() {
  var relatedProductsContainer = $(this).closest('.product_block.plateOuter.forMobile').find('.related-products');
  relatedProductsContainer.animate({
  scrollLeft: '+=' + (relatedProductsContainer.find('li').width() + 10) 
  }, 500); 
  });
  
  $(".online-status").on('click',function() {
...
$(document).ready(function() {
  if (window.innerWidth > 768) {    
    $('.top-bar__slider .top-bar__slide:eq(1)').remove();    
    var flkty = new Flickity('.top-bar__slider');
    console.log('Disable the entire Flickity slider');
    flkty.destroy();
  }

      $('.accordionSet section').click(function(){
      $(this).toggleClass('toggleSection');
    });
  
});

    $('a.navlink.navlink--child.navlink--image.has-items').on('click', function(event) {
    event.preventDefault(); // Prevent the default behavior of the link

    // Hide all dropdowns with class 'dropdown__family'
    $('.dropdown__family.czfam').toggleClass('hideCZ');

    // Show the parent dropdown of the clicked link
    $(this).closest('.dropdown__family.czfam').show();
    $(this).toggleClass('hide-after');
    // Show the next '.subchildCZ' element
    $(this).next('.subchildCZ').toggleClass('showCZ');
});  

$(document).on("click", ".mobile-menu__item.mobile-menu__item--level-2.mobile-navlink--trigger", function(event) {
    event.preventDefault();
    // Toggle the class only for the clicked element
    $(this).find("a.mobile-navlink.mobile-navlink--level-1").toggleClass("mobileAfterCZ");
});
   ...
   $('.bcorp_link').on('click', function() {
     window.open("https://www.bcorporation.net/en-us/find-a-b-corp/company/nordic-apparel-ap-s/", "_blank");
  });
   $('.pop_close').on('click', function() {
    $('.select-popout__toggle').trigger('click');
  });
       $(".toTopBtn").on("click", function(e) {
        e.preventDefault();
        $("html, body").animate({ scrollTop: 0 }, 500);
      });
      $('.non-redirect').click(function(event) {
              event.preventDefault();
        var divPosition = $('#vegetable-tanned').offset().top - 120;
